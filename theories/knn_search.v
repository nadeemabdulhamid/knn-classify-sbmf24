...
*)    
Fixpoint knn
 (dist_metric:datapt -> datapt -> nat)
 (K:nat) (k:nat) (tree:kdtree) (bb:bbox) (query:datapt) (pq:priqueue datapt (dist_metric query)) 
  : priqueue datapt (dist_metric query) 
  := match tree with 
        | mt_tree => pq
        | node ax pt lft rgt =>
            let body (pq':priqueue datapt (dist_metric query)) := 
                ...
                then (knn dist_metric K k rgt (snd bbs) query
                        (knn dist_metric K k lft (fst bbs) query pq'))
                else (knn dist_metric K k lft (fst bbs) query
                        (knn dist_metric K k rgt (snd bbs) query pq')) 
            in
            match (peek_max _ _ pq) with
            | None => body (insert_bounded K _ _ pt pq)
            | Some top => if (K <=? (size _ _ pq))
                                && ((dist_metric query top) <? (dist_metric query (closest_edge_point query bb)))
                        then pq
                        else body (insert_bounded K _ _ pt pq)
            end
     end.

Definition knn_search
    (dist_metric:datapt -> datapt -> nat)
    (K:nat) (k:nat) (tree:kdtree) (query:datapt) : list datapt :=
   pq_to_list 
     (knn dist_metric K k tree (mk_bbox (repeat None k) (repeat None k)) query (empty datapt (dist_metric query))).
...
Proof.
    intros; repeat split.
    apply all_in_leb_app_head_1 with lstSm; apply all_in_leb_app_tail_1 with lstLg;
        apply (all_in_leb_perm _ _ result _ leftover); auto.
    apply all_in_leb_app_head_1 with lstSm; apply all_in_leb_app_tail_2 with eLg;
        apply (all_in_leb_perm _ _ result _ leftover); auto.
    apply all_in_leb_app_head_2 with eSm; apply all_in_leb_app_tail_1 with lstLg;
        apply (all_in_leb_perm _ _ result _ leftover); auto.
    apply all_in_leb_app_head_2 with eSm; apply all_in_leb_app_tail_2 with eLg; 
        apply (all_in_leb_perm _ _ result _ leftover); auto.
Qed.
...
Proof.
    ...
    destruct (K <? size A key (insert A key e pq)) eqn:Hlt; split_andb_leb.
    {
        ...
        inversion_clear Hdm'; split_andb_leb.
        {
            ...
            intros; rewrite insert_size with (al := lst) in Hlt; auto; try lia.
            intros; exists e; rewrite app_nil_r; auto.
        }
        {
            pose proof Hdm as Hdm';
            apply delete_max_Some_relate with (pl := e::lst) (ql := ql) in Hdm' as (Hdms, Hcomp); auto using insert_priq, insert_relate;
    ...
    }
    {
        ...
        exists lst; exists [].
        exists (e :: lst); exists [].
        repeat split; try rewrite app_nil_r; auto.
        rewrite insert_size with (al := lst) in Hlt; auto; try lia.
    }
Qed.
...
Lemma knn_relate :
    ...
    (Hpriq : priq datapt (dist_metric query) pq)
    (Habs : Abs _ _ pq lst),
    exists lst', priq _ _ (knn dist_metric K k tree bb query pq) /\ Abs _ _ (knn dist_metric K k tree bb query pq) lst'.
Proof.
    ...
    destruct (IHlft (fst (bb_split bb ax (nth ax pt 0))) query 
            (insert_bounded K datapt (dist_metric query) pt pq)
                lsti) as (lstLft, (Hlft1, Hlft2)); auto.
    destruct (IHrgt (snd (bb_split bb ax (nth ax pt 0))) query 
                (insert_bounded K datapt (dist_metric query) pt pq)
                    lsti) as (lstRgt, (Hrgt1, Hrgt2)); auto.
    simpl.
    destruct (peek_max datapt (dist_metric query) pq) as [ top | ] eqn:Hpeek.
    1: {
        destruct ((K <=? size datapt (dist_metric query) pq) &&
                ((dist_metric query) top <? (dist_metric query) (closest_edge_point query bb))).
        eauto.
...
Lemma knn_search_build_size_gen :
    ...
    (Hpriq : priq _ (dist_metric query) pq)
    (Habs  : Abs _ (dist_metric query) pq lst)
    (Htree : Contents_kdtree tree data)
    (Hpqsz : size datapt _ pq <= K),
    size datapt _ (knn dist_metric K k tree bb query pq) = (min K ((length data) + (size datapt _ pq))).
Proof.
    intros dist_metric .
    induction tree as [ | ax pt lft IHlft rgt IHrgt ].
    - 
    ...
    pose proof (knn_relate _ K k lft (fst (bb_split bb ax (nth ax pt 0))) query
    (insert_bounded K datapt (dist_metric query) pt pq) lst' Hpriq' Habs') as 
        (lst'', (Hpriq'', Habs'')).
    pose proof (knn_relate _ K k rgt (snd (bb_split bb ax (nth ax pt 0))) query
        (insert_bounded K datapt (dist_metric query) pt pq) lst' Hpriq' Habs') as 
            (lst''', (Hpriq''', Habs''')); auto.

    destruct (peek_max datapt (dist_metric query) pq) as [ top | ] eqn:Hpeek.
    --  
        destruct (K <=? size datapt (dist_metric query) pq) eqn:Hksz; simpl.
        destruct ((dist_metric query) top <? (dist_metric query) (closest_edge_point query bb)) eqn:Hsd.
        split_andb_leb; simpl; lia.
        split_andb_leb.
        assert (size datapt (dist_metric query) pq = K); try lia.
        ...
        split_andb_leb.
        apply (insert_bounded_lt_preserve_max) with (A := datapt) (e := pt) (lst := lst) (key := (dist_metric query)) in Hksz  as Hib; eauto. 
        destruct (ith_leb ax pt query) eqn:Hith.
        ---
    ...
    --
    eapply insert_bounded_le_preserve_max with (A:=datapt) (e:=pt) in Hpqsz as Hible; eauto.
    ...
    assert (size datapt _ pq = 0) as Hsz.
    1: {
        assert (Abs _ _ pq []).
        apply peek_max_None_relate; auto.
        apply size_zero_relate; auto.
    }
    assert (size datapt _ pq < K) as Hszltk; try lia.
    eapply insert_bounded_lt_preserve_max with (A:=datapt) (e:=pt) in Hszltk as Hib; eauto.
...
Lemma knn_search_build_size :
    ...
    (Hk : 0 < k),
    size datapt (dist_metric query)
        (knn dist_metric K k (build_kdtree k data)
...
Proof.
    ...
    assert (Abs _ _ (empty datapt (dist_metric query)) []) as Habs.
    1: { apply empty_relate; auto. }
    assert (size datapt (dist_metric query) (empty datapt (dist_metric query)) = 0).
...
Lemma knn_preserve_size_max :
    ...
    (Hpriq : priq datapt (dist_metric query) pq)
    (Habs : Abs _ _ pq lst)
    (Heq_Ksize: K <= size datapt _ pq),
    size datapt _ pq = size datapt _ (@knn dist_metric K k tree bb query pq).
Proof.
    ...
    assert (K <= size datapt _ (insert_bounded K datapt _ pt pq)); try lia.
    { erewrite <- insert_bounded_preserve_max_eq; eauto. }
    assert (size datapt _ pq = size datapt _ (insert_bounded K datapt _ pt pq)).
    { erewrite <- insert_bounded_preserve_max_eq; eauto. }

    assert (size datapt _ pq =
        size datapt _
          (knn dist_metric K k rgt (snd (bb_split bb ax (nth ax pt 0))) query
             (knn dist_metric K k lft (fst (bb_split bb ax (nth ax pt 0))) query (insert_bounded K datapt _ pt pq))))
            as Hsz_knn_lft_rgt.         
    {
        pose proof (knn_relate _ K k lft (fst (bb_split bb ax (nth ax pt 0))) query _ _ Hp1 Ha1)
            as (pqlstLft, (Hp2, Ha2)).
        pose proof (knn_relate _ K k rgt (snd (bb_split bb ax (nth ax pt 0))) query _ _ Hp2 Ha2)
            as (pqlstRgt, (Hp3, Ha3)).
    ...
    assert (size datapt (dist_metric query) pq =
    size datapt (dist_metric query)
      (knn dist_metric K k lft (fst (bb_split bb ax (nth ax pt 0))) query
         (knn dist_metric K k rgt (snd (bb_split bb ax (nth ax pt 0))) query (insert_bounded K datapt (dist_metric query) pt pq)))
    ) as Hsz_knn_rgt_lft.
    {
        pose proof (knn_relate dist_metric K k rgt (snd (bb_split bb ax (nth ax pt 0))) query _ _ Hp1 Ha1)
            as (pqlstRgt, (Hp2, Ha2)).
        pose proof (knn_relate dist_metric K k lft (fst (bb_split bb ax (nth ax pt 0))) query _ _ Hp2 Ha2)
            as (pqlstLft, (Hp3, Ha3)).
            erewrite <- IHlft; eauto.
            erewrite <- IHrgt; eauto.
            erewrite <- IHrgt; eauto.
    ...
    destruct (peek_max datapt (dist_metric query) pq) eqn:Heq1; [ | destruct (ith_leb ax pt query) eqn:Heq3 ]; auto.
    destruct ((K <=? size datapt (dist_metric query) pq) && ((dist_metric query) d <? (dist_metric query) (closest_edge_point query bb))) eqn:Heq2; auto.
...
Lemma knn_preserve_size_lt_inv :
    forall dist_metric K k tree bb query pq lst data
    (Hpriq : priq datapt (dist_metric query) pq)
    (Habs : Abs datapt _ pq lst)
    (Hcont : Contents_kdtree tree data)
    (HltK : size datapt (dist_metric query) (knn dist_metric K k tree bb query pq) < K),
    size datapt _ pq < K.
Proof.
    intros.
    destruct (Nat.lt_ge_cases (size datapt _ pq) K); auto.
...
Lemma knn_full_relate_gen :
    forall dist_metric,
      dist_metric_wf dist_metric ->
    forall K k tree bb query pq (lst data result:list datapt)
    (HK : 0 < K)
    (Hb : kdtree_bounded tree bb = true)
    (Htree : Contents_kdtree tree data)
    (Hpriq : priq datapt (dist_metric query) pq)
    (Habs : Abs _ (dist_metric query) pq lst)
    (Hpriq' : priq _ _ (knn dist_metric K k tree bb query pq))
    (Habs' : Abs _ _ (knn dist_metric K k tree bb query pq) result),

    exists (lstSm lstLg dataSm dataLg leftover : list datapt),
        Permutation (lstSm ++ lstLg) lst /\
        Permutation (dataSm ++ dataLg) data /\
        Permutation (lstSm ++ dataSm) result /\
        Permutation (lstLg ++ dataLg) leftover /\
        (size _ _ (knn dist_metric K k tree bb query pq) < K -> leftover = []) /\
        all_in_leb (dist_metric query) result leftover.
Proof.
    intros sum_dist Hdmwf.
    ...
    { (* peek is some *)
        ...
        { (* K <= size pq *)
            assert (size datapt (sum_dist query) (knn sum_dist K k (node ax pt lft rgt) bb query pq) < K -> data = [])
                    as Hnon_applicable.
            {
                split_andb_leb.
                intros Hcontra.
                rewrite <- (knn_preserve_size_max sum_dist 
                K k (node ax pt lft rgt) bb query pq lst) in Hcontra; auto. lia.
            }
            ...
            { (* closest edge point is farther than the largest thing in the pq *)
                split_andb_leb.
                assert (Permutation lst result); [ apply abs_perm with (key:=sum_dist query) (p:=pq); auto | ].
                ...
                apply closest_edge_point_min_dist; auto.
            ...
            { (* closest edge point is potentially closer than the top (largest thing) in the pq *)
                ...
                { (* ith = true --- insert_bounded (maxed out) then lft then rgt *)
                    destruct (knn_relate sum_dist K k lft (fst (bb_split bb ax (nth ax pt 0))) query 
                                        (insert_bounded K datapt (sum_dist query) pt pq) _ H1 H2) as
                            (pqlstLft, (Hplft, Halft)).
                    destruct (knn_relate sum_dist K k rgt (snd (bb_split bb ax (nth ax pt 0)))
                    ...
                    { apply abs_perm with (sum_dist query)
                            (knn sum_dist K k rgt (snd (bb_split bb ax (nth ax pt 0))) query
                            (knn sum_dist K k lft (fst (bb_split bb ax (nth ax pt 0))) query (insert_bounded K datapt (sum_dist query) pt pq))); auto.
                    }
...

                    {
                        ...
                        assert (K <= length pqlstLft) as H60.
                        {
                            ...
                            rewrite <- size_relate with (p:=pq); auto.
                            rewrite <- size_relate with (p:=(knn sum_dist K k lft (fst (bb_split bb ax (nth ax pt 0))) query
                        ...
                        assert (length pqlstLft = length pqlstRgt) as H61.
                        {
                            rewrite <- size_relate with (p:=(knn sum_dist K k lft (fst (bb_split bb ax (nth ax pt 0))) query
                                (insert_bounded K datapt (sum_dist query) pt pq))); auto.
                            rewrite <- size_relate with (p:=(knn sum_dist K k rgt (snd (bb_split bb ax (nth ax pt 0))) query
                                (knn sum_dist K k lft (fst (bb_split bb ax (nth ax pt 0))) query
                                   (insert_bounded K datapt (sum_dist query) pt pq)))); auto.
                            erewrite knn_preserve_size_max; eauto.
                            erewrite <- knn_preserve_size_max; eauto.
                        }
                        ...
                        { (* pqlstRgt <= leftoveri*)
                            ...
                            { (* lresSm <= leftoveri *)
                                ...
                                -
                                rewrite Hd1 in *; repeat rewrite app_nil_r in *; rewrite app_length in *.
                                assert (lfttreeSm = []) as Hd3.
                                {
                                    destruct lfttreeSm; auto; simpl in H68; lia.
                                }
                                rewrite Hd3 in *; repeat rewrite app_nil_r in *; simpl in *.
                            ...
                            { (* rgttreeSm <= leftoveri *)
                                ...
                                {   (* resiLg = [] *)
                                    ...
                                    -
                                    rewrite H18 in *; simpl in *; repeat rewrite app_nil_r in *.
                                    clear H69 H95 H90.
                                    destruct H67 as (H67a, H67b).
                                    assert (Permutation resiSm (lresSm ++ lresLg)).
                                    permtrans pqlstLft; auto.
                                    destruct (list_nil_or_in lresLg) as [ Habsurd | Hd2 ].
                                    { rewrite Habsurd in H62; rewrite app_nil_r in H62; repeat rewrite app_length in H62; simpl in H62.
                ...
                { (* ith = false --- rgt then lft *)
                    (* apply MAGIC. *)
                    destruct (knn_relate sum_dist K k rgt (snd (bb_split bb ax (nth ax pt 0))) query 
                                        (insert_bounded K datapt (sum_dist query) pt pq) _ H1 H2) as
                            (pqlstLft, (Hplft, Halft)).
                    destruct (knn_relate sum_dist K k lft (fst (bb_split bb ax (nth ax pt 0)))
                                        query _ _ Hplft Halft) as (pqlstRgt, (Hprgt, Hargt)).
                    pose proof Hb as Hb'; simpl in Hb'; split_andb_leb.
                    destruct (cont_node_inv _ _ _ _ _ Htree) as (lftConts, (rgtConts, (Hcontlft, (Hcontrgt, Hconteq)))).
                    destruct (IHrgt _ _ _ _ _ _
                    ...
                    { apply abs_perm with (sum_dist query)
                            (knn sum_dist K k lft (fst (bb_split bb ax (nth ax pt 0))) query
                            (knn sum_dist K k rgt (snd (bb_split bb ax (nth ax pt 0))) query (insert_bounded K datapt (sum_dist query) pt pq))); auto.
                    }
        ...
        { (* size pq < K*)
            ...
            { (* ith = true --- lft then rgt *)
                destruct (knn_relate sum_dist K k lft (fst (bb_split bb ax (nth ax pt 0))) query pq' _ H1 H2) 
                    as (pqlstLft, (Hplft, Halft)).
                destruct (knn_relate sum_dist K k rgt (snd (bb_split bb ax (nth ax pt 0))) query _ _ Hplft Halft) 
                ...
                { apply abs_perm with (sum_dist query)
                        (knn sum_dist K k rgt (snd (bb_split bb ax (nth ax pt 0))) query
                        (knn sum_dist K k lft (fst (bb_split bb ax (nth ax pt 0))) query pq')); auto. }
                ...
                (knn sum_dist K k rgt (snd (bb_split bb ax (nth ax pt 0))) query
                (knn sum_dist K k lft (fst (bb_split bb ax (nth ax pt 0))) query pq')) < K -> loLft = [] /\ loRgt = []).
                { (* size / leftover empty *)
                    intros HltK.
                ...
                { (* pqlstRgt <= loLft ++ loRgt *)
                    ...
                    destruct (Nat.lt_ge_cases (size datapt (sum_dist query)
                        (knn sum_dist K k rgt (snd (bb_split bb ax (nth ax pt 0))) query
                                    (knn sum_dist K k lft (fst (bb_split bb ax (nth ax pt 0))) query pq')))
                        K); auto.
                    {
                    ...
                    destruct (Nat.lt_ge_cases (size datapt (sum_dist query) (knn sum_dist K k lft (fst (bb_split bb ax (nth ax pt 0))) query pq'))
                                        K); auto.
                    {
                    ...
                    { 
                        rewrite (Permutation_length Hr1). rewrite (Permutation_length Hr3); auto.
                        rewrite <- size_relate with (p:=(knn sum_dist K k lft (fst (bb_split bb ax (nth ax pt 0))) query pq')); auto.
                        rewrite <- size_relate with (p:=(knn sum_dist K k rgt (snd (bb_split bb ax (nth ax pt 0))) query
                                        (knn sum_dist K k lft (fst (bb_split bb ax (nth ax pt 0))) query pq'))); auto.
                        erewrite knn_preserve_size_max; eauto.
                    }
            ...
            { (* ith = false --- rgt then lft *)
                (* apply MAGIC.  *)
                destruct (knn_relate sum_dist K k rgt (snd (bb_split bb ax (nth ax pt 0))) query pq' _ H1 H2) 
                    as (pqlstLft, (Hplft, Halft)).
                destruct (knn_relate sum_dist K k lft (fst (bb_split bb ax (nth ax pt 0))) query _ _ Hplft Halft) 
                    as (pqlstRgt, (Hprgt, Hargt)).
                ...
                { apply abs_perm with (sum_dist query)
                        (knn sum_dist K k lft (fst (bb_split bb ax (nth ax pt 0))) query
                        (knn sum_dist K k rgt (snd (bb_split bb ax (nth ax pt 0))) query pq')); auto. }
                ...
                assert (size datapt (sum_dist query)
                (knn sum_dist K k lft (fst (bb_split bb ax (nth ax pt 0))) query
                (knn sum_dist K k rgt (snd (bb_split bb ax (nth ax pt 0))) query pq')) < K -> loLft = [] /\ loRgt = []).
                { (* size / leftover empty *)
                    intros HltK.
                    apply Hr6 in HltK as Hr6'.
                    eapply knn_preserve_size_lt_inv in HltK as HltK'; eauto.
                ...
                { (* pqlstRgt <= loLft ++ loRgt *)
                    ...
                    destruct (Nat.lt_ge_cases (size datapt (sum_dist query)
                        (knn sum_dist K k lft (fst (bb_split bb ax (nth ax pt 0))) query
                                    (knn sum_dist K k rgt (snd (bb_split bb ax (nth ax pt 0))) query pq')))
                        K); auto.
                    {
                        apply H12 in H13 as (Hsz1, Hsz2).
                        replace loLft with (@nil datapt); auto.
                    ...
                    destruct (Nat.lt_ge_cases (size datapt (sum_dist query) (knn sum_dist K k rgt (snd (bb_split bb ax (nth ax pt 0))) query pq'))
                                        K); auto.
                    {
                        apply Hl6 in H14. replace loLft with (@nil datapt); auto.
                    }
                    ...
                    { 
                        rewrite (Permutation_length Hr1). rewrite (Permutation_length Hr3); auto.
                        rewrite <- size_relate with (p:=(knn sum_dist K k rgt (snd (bb_split bb ax (nth ax pt 0))) query pq')); auto.
                        rewrite <- size_relate with (p:=(knn sum_dist K k lft (fst (bb_split bb ax (nth ax pt 0))) query
                                        (knn sum_dist K k rgt (snd (bb_split bb ax (nth ax pt 0))) query pq'))); auto.
                        erewrite knn_preserve_size_max; eauto.
                    }
...
Lemma knn_full_relate :
    ...
    forall pq, 
    pq = (knn dist_metric K k (build_kdtree k data) (mk_bbox (repeat None k) (repeat None k)) query
                                            (empty datapt (dist_metric query))) ->
    exists result leftover : list datapt,
        Permutation data (result ++ leftover) /\
        Abs datapt (dist_metric query) pq result /\
        all_in_leb (dist_metric query) result leftover.
Proof.
    intros dist_metric Hdmwf K k data query HK Hlen Hk Hd pq Hpq.
    ...
    assert (Abs datapt (dist_metric query) (empty datapt (dist_metric query)) []).
    apply empty_relate; auto.

    destruct (knn_relate dist_metric K k (build_kdtree k data) (mk_bbox (repeat None k) (repeat None k))
    query (empty datapt (dist_metric query)) []) as (result, (H1, H2)); auto.

    destruct (knn_full_relate_gen dist_metric Hdmwf K k (build_kdtree k data) (mk_bbox (repeat None k) (repeat None k))
    query (empty datapt (dist_metric query)) [] data' result HK) 
...
Theorem knn_search_build_kdtree_correct :
    ...
    knn_search dist_metric K k tree query = result ->
    exists leftover, 
        length result = min K (length data)
        /\ Permutation data (result ++ leftover)
        /\ all_in_leb (dist_metric query) result leftover.
Proof.
    intros dist_metric Hdmwf K k data HK Hlen Hk Hin tree query result Htree Hknn.
    rewrite Htree in *; clear Htree tree.
    unfold knn_search in Hknn.
    pose proof (build_kdtree_contents_perm k data Hk) as (lst, (Hc, Hp)).
    
    assert (priq _ _ (knn dist_metric K k (build_kdtree k data) (mk_bbox (repeat None k) (repeat None k)) query
        (empty datapt (dist_metric query)))) as Hpriq.
    {
        destruct (knn_relate dist_metric K k (build_kdtree k data) (mk_bbox (repeat None k) (repeat None k)) query
        (empty datapt (dist_metric query)) []) as (lst', (Hpriq, Habs)); auto.
        apply empty_relate; auto.
    }

    assert (exists top, exists leftover,
                Permutation data (top ++ leftover) /\
                (Abs _ _ (knn dist_metric K k (build_kdtree k data) (mk_bbox (repeat None k) (repeat None k)) query
                        (empty datapt (dist_metric query))) top) /\
                all_in_leb (dist_metric query) top leftover)
    as (top, (leftover, (Hperm, (Habs, Hle)))).
    {
        apply (knn_full_relate dist_metric Hdmwf K k data query HK Hlen Hk Hin (knn dist_metric K k (build_kdtree k data) (mk_bbox (repeat None k) (repeat None k)) query
    (empty datapt (dist_metric query)))); auto.
    }
        
    pose proof (pq_to_list_relate _ _ _ _ _ Hpriq Habs Hknn).
    exists leftover; repeat split.
    -   apply size_relate in Habs as Hsz; auto.
        pose proof (knn_search_build_size dist_metric Hdmwf K k data query Hk).
...
Example ex1 : (rev (knn_search manhattan_dist 3 2 two_tree [ 5 ; 5 ])) = [[4; 3]; [5; 10]; [8; 2]]  := refl_equal.
Example ex2 : (rev (knn_search manhattan_dist 1 2 two_tree [ 5 ; 5 ])) = [[4; 3]] := refl_equal.
Example ex3 : (rev (knn_search manhattan_dist 5 2 two_tree [ 5 ; 5 ])) = [[4; 3]; [5; 10]; [8; 2]; [7; 12]; [9; 10]] := refl_equal.
