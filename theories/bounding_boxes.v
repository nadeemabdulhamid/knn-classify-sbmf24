...
(* NOTE: TODO:  is this just finding the median using a decision tree? .... *)
...
Fixpoint manhattan_dist (vs ws:list nat) : nat :=
    match vs, ws with 
    | [], [] => 0
    | x :: vs', y :: ws' => abs_diff x y + manhattan_dist vs' ws'
    | _, _ => 0
    end.


Example ex8 : manhattan_dist [10 ; 20 ; 5] [15 ; 12 ; 8] = 5+8+3 := refl_equal.


Definition dist_metric_wf (dist_metric : list nat -> list nat -> nat) :=
    (forall vs, dist_metric [] vs = 0) /\
    (forall vs, dist_metric vs [] = 0) /\
    (forall v w p q vs ws ps qs,
        abs_diff v w <= abs_diff p q ->
        dist_metric vs ws <= dist_metric ps qs ->
        dist_metric (v :: vs) (w :: ws) <= dist_metric (p :: ps) (q :: qs)).


(*        
Definition tri_ineq (dist_metric : list nat -> list nat -> nat) :=
    forall vs ws xs,
        dist_metric vs xs <= dist_metric vs ws + dist_metric ws xs.

Lemma tri_ineq_dist_metric_wf : 
    forall D, tri_ineq D -> 
        (forall xs ys, D xs ys = D (0 :: xs) (0 :: ys)) ->
        (forall v w p q vs ws ps qs,
            abs_diff v w <= abs_diff p q ->
            D vs ws <= D ps qs ->
            D (v :: vs) (w :: ws) <= D (p :: ps) (q :: qs)).
Proof.
...
Lemma manhattan_dist_wf : dist_metric_wf manhattan_dist.
Proof.
    repeat split.
    destruct vs; auto.
    destruct vs; auto.
...
Lemma cep_min_dist :
    ...
    forall vs mins maxs ws , 
        cep vs mins maxs = ws -> 
            ...
            dist_metric vs ws <= dist_metric vs pt.
Proof.
    intros dist_metric (Hdm1, (Hdm2, Hdm3)).
    induction vs as [ | v vs']; simpl; intros; try (rewrite <- H; rewrite Hdm1; lia).
    destruct mins as [ | jo mins' ]; try (rewrite <- H; rewrite Hdm2; lia).
    destruct maxs as [ | ko maxs' ]; try (rewrite <- H; rewrite Hdm2; lia).
    destruct ws as [ | w ws']; try (rewrite Hdm2; lia).
    ...
    2: {
        assert (dist_metric vs' ws' <= dist_metric vs' pt'); 
            try (apply Hdm3; auto).
...
Lemma closest_edge_point_min_dist :
    ...
    forall bb pt pt', 
        ...
        dist_metric pt (closest_edge_point pt bb) <= dist_metric pt pt'.
Proof.
