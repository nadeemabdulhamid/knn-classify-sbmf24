...
Ltac count_down := fun x => 
    let name := (fresh "x") in 
    match x with 0 => set (name := 0)
    | S ?t => set (name := x) ; count_down t
    end.

(*  count_pieces_work : term nat (nat -> ())  *)
Ltac count_pieces_work term n cont :=
    (* idtac term; *)
    match goal with
    | current_counter := ?cc |- _ =>
        match term with 
        | context f [?X ++ ?Y] => 
                count_pieces_work X n 
                    ltac:(fun n' => count_pieces_work Y n' cont)
        | ?X =>
            match goal with
            | _ := (?xn, X) |- _ => 
                (* let name := fresh "x" in set (name := (xn, X)); *)
                (cont (S n))
            | _ =>
                let name := fresh "x" in set (name := (cc, X));
                        clear current_counter;
                        set (current_counter := S cc);
                        (cont (S n))
            end
        end
    end.
...
Ltac count_pieces :=
    set (current_counter := 10);
    match goal with

    | |- context f [Permutation (?X ++ ?Y) (?S ++ ?T)]
     => count_pieces_work (X ++ Y) 0   (* ltac:(fun _ => idtac) *)
            ltac:(fun n' => count_pieces_work (S ++ T) n' ltac:(fun _ => idtac))
               (*  ltac:(fun n' => count_pieces n')  ltac:(fun _ => idtac) *)
    end.




Lemma stub : forall A (a:list A) b c d e f g, 
    Permutation ((rev a) ++ (b ++ c) ++ (d ++ a))
                ((e ++ f) ++ (g ++ b) ++ (rev a)).
    intros.
    count_pieces.
    ...
    match goal with 
    | |- context f [?X ++ ?Y] =>
        let name := fresh "x" in set (name := X) 
    end.
...
Parameter a : list nat.
Parameter b : list nat.
