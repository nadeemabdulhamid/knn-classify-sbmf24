...
(* NOTE:
  ...
  This hardcodes the manhattan distance as the distance metric. 
  See knn_search for generalization to any "well-formed" distance metric.
*)
...
Fixpoint nn (k:nat) (tree:kdtree) (bb:bbox)
    ...
    | node ax pt lft rgt => 
       let closer := (match besto with (* check the tree root against current best candidate, if any *)
                        | None => pt
                        | Some best => if (manhattan_dist goal pt) <? (manhattan_dist goal best) then pt else best
                      end) in
        ...
        in
        match besto with
            | Some best => if ((manhattan_dist goal best) <=? (manhattan_dist goal (closest_edge_point goal bb)))
                           then besto
                           else (body closer)
            | None => (body closer)
        end
...
Proof.
    ...
    simpl; intros ; eauto.
    simpl; intros.
    destruct (manhattan_dist goal best <=? manhattan_dist goal (closest_edge_point goal bb)) eqn:Heq1; eauto.
    destruct (ith_leb ax pt goal) eqn:Heq2; eauto.
    - destruct (IHlft (fst (bb_split bb ax (nth ax pt 0))) 
                    goal 
                    (if manhattan_dist goal pt <? manhattan_dist goal best then pt else best)) as [ v Hlft ]; rewrite Hlft.
    ...
    - destruct (IHrgt (snd (bb_split bb ax (nth ax pt 0)))
                      goal 
                      (if manhattan_dist goal pt <? manhattan_dist goal best then pt else best))  as [ v Hrgt ]; rewrite Hrgt.
...
Proof.
    ...
    destruct (manhattan_dist goal best <=? manhattan_dist goal (closest_edge_point goal bb)) eqn:Hd1.
    ...
    destruct (nn_some_result k lft (fst (bb_split bb ax (nth ax pt 0))) goal
        (if manhattan_dist goal pt <? manhattan_dist goal best then pt else best)) as
    ...
    -- 
        rewrite H0.
        destruct (manhattan_dist goal pt <? manhattan_dist goal best).
    ...
    destruct (nn_some_result k rgt (snd (bb_split bb ax (nth ax pt 0))) goal
        (if manhattan_dist goal pt <? manhattan_dist goal best then pt else best)) as [vr Hvr]; rewrite Hvr in H.
    ...
    inversion_clear Hvl.
    -- rewrite H0.
        destruct (manhattan_dist goal pt <? manhattan_dist goal best); inversion_clear Hvr.
        right; left; auto.
        right; right; right; auto.
        left; auto.
        right; right; right; auto.
...
Lemma nn_result_le : forall k tree bb goal best v,
    nn k tree bb goal (Some best) = Some v -> manhattan_dist goal v <= manhattan_dist goal best.
Proof.
    induction tree as [ | ax pt lft IHlft rgt IHrgt ].
    1: { simpl; intros. injection H as H; rewrite H; auto. }
    simpl; intros bb goal best v Hnn.
    destruct (manhattan_dist goal best <=? manhattan_dist goal (closest_edge_point goal bb)) eqn:He1.
    injection Hnn as Hnn; rewrite Hnn; auto.
    destruct (ith_leb ax pt goal) eqn:He2;
        destruct (manhattan_dist goal pt <? manhattan_dist goal best) eqn:He3;
...
Lemma nn_correct_1 : forall k tree bb goal besto result,
    ...
    manhattan_dist goal result <= manhattan_dist goal v'.
Proof.
    induction tree as [ | ax pt lft IHlft rgt IHrgt ].
    ...
    2: { 
        ...
        - rewrite <- Hin_pt in *; clear Hin_pt v'.
          ...
          -- 
            ...
            apply Nat.le_trans with (manhattan_dist goal vl);
          ...
          -- destruct (nn_some_result k rgt (mk_bbox (list_set bb_min ax (Some (nth ax pt 0))) bb_max) goal pt) 
            ...
            apply Nat.le_trans with (manhattan_dist goal vr); eapply nn_result_le; eauto.
        - simpl in Hnn.
          destruct (ith_leb ax pt goal) eqn:Hleb_ax.
            -- destruct (nn_some_result k lft (mk_bbox bb_min (list_set bb_max ax (Some (nth ax pt 0)))) goal pt)
                as [vl Hvl]; rewrite Hvl in Hnn.
        ...
        -- destruct (nn_some_result k rgt (mk_bbox (list_set bb_min ax (Some (nth ax pt 0))) bb_max) goal pt)
            as [vr Hvr]; rewrite Hvr in Hnn.
            apply Nat.le_trans with (manhattan_dist goal vr).
            eapply nn_result_le; eauto.
            eapply IHrgt; eauto.
    ...
    1: {
        ...
        destruct (manhattan_dist goal best <=? manhattan_dist goal (cep goal bb_min bb_max)) eqn:Heq3; split_andb_leb.
        --  injection Hnn as Hnn; rewrite Hnn in *; clear best Hnn.
            (* so result is closer than anything in the bb of this tree *)
            apply Nat.le_trans with (manhattan_dist goal (cep goal bb_min bb_max)); auto.
            apply (cep_min_dist _ manhattan_dist_wf goal bb_min bb_max); auto.
        ...
        --- (* considering v' = the datapt at the root of the tree *)
            ...
            destruct (ith_leb ax pt goal) eqn:Hleb; (* COND (3) *)
                ...
                destruct (manhattan_dist goal pt <? manhattan_dist goal best) eqn:Hcond2; (* COND (2) *)
                    split_andb_leb; lia.
...
Theorem nn_search_correct : 
    ...
    exists result, 
        ...
        /\ In_kdtree result tree
        /\ (forall v', In_kdtree v' tree -> manhattan_dist goal result <= manhattan_dist goal v').
Proof.
...
Theorem nn_search_build_kdtree_correct :
    ...
    tree = (build_kdtree k data) ->
    nn_search k tree goal = Some result ->
       In result data /\ (forall v', In v' data -> manhattan_dist goal result <= manhattan_dist goal v').
Proof.
...
Theorem nn_search_build_kdtree_always_result_correct :
    ...
    exists result, 
        nn_search k tree goal = Some result
        /\ In result data
        /\ (forall v', In v' data -> manhattan_dist goal result <= manhattan_dist goal v').
Proof.
